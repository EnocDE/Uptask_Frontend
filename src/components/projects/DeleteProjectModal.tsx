...
export default function DeleteProjectModal() {
  ...
  // Se hizo asi para estetica del reinicio solamente
  useEffect(() => {
    if (show) {
      reset()
    }
  ...
  const deleteProjectMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // No invalidaba el cache si se eliminaba el ultimo proyecto
      queryClient.clear()
      toast.success(data)
  ...
  const handleForm = async (formData: CheckPasswordForm) => {
    // Usa mutateAsync para esperar la mutación, si da un error no ejecuta el código siguiente
            ...
            >
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all p-16">
                ...
                <p className="text-xl font-bold">Confirma la eliminación del proyecto {''}
                  <span className="text-fuchsia-600">colocando la contraseña de tu cuenta</span>
                </p>
